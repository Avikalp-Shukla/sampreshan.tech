...
if ( ! function_exists( 'buddyboss_theme_bb_blog_is_active' ) ) {
	/**
	 * Whether the Platform "blogging" feature is active.
	 *
	 * Blog is a Settings 2.0 feature (id `blogging`), not a legacy BP
	 * component, so there is no `bp_is_active( 'blog' )` to call -- the
	 * feature registry is the only source of truth.
	 *
	 * @since 2.21.0
	 *
	 * @return bool
	 */
	function buddyboss_theme_bb_blog_is_active() {
		return function_exists( 'bb_feature_registry' )
			&& bb_feature_registry()->bb_is_feature_active( 'blogging' );
	}
}

if ( ! function_exists( 'buddyboss_theme_bb_blog_single_actions' ) ) {
	/**
	 * Output the Subscribe and Bookmark actions for a single blog post.
	 *
	 * Rendered into the post meta row (`.post-meta-wrapper-main`) rather than
	 * reusing Pro's own renderers, because those hardcode `bb-icons-rl-*`
	 * glyphs and that font (`bb-icons-rl-css`) is only enqueued in ReadyLaunch
	 * mode -- in classic theme mode they would render as blank tofu. The
	 * markup below uses the theme's own icon font while keeping every class
	 * and data attribute Pro's delegated JS binds to:
	 *
	 * - `.js-bb-blog-subscribe-open` opens the "Subscribe to topics" modal
	 *   (bb-blog-subscribe-modal.js). The modal shell and its script are
	 *   already loaded on `is_singular( 'post' )` by Pro's own modal context.
	 * - `.bb-blog-bookmark` + `data-post-id` + `aria-pressed` drive the
	 *   bookmark toggle (bb-blog-bookmarks.js). That script only flips the
	 *   `is-bookmarked` class, so the theme icon is safe.
	 *
	 * Both actions are gated on the Platform blogging feature being active and
	 * on their own toggle in Blog Settings -> Post Settings, via Pro's
	 * `bb_blog_pro_is_*_enabled()` helpers (which also enforce the license).
	 *
	 * @since 2.21.0
	 *
	 * @return void
	 */
	function buddyboss_theme_bb_blog_single_actions() {
		if ( ! is_singular( 'post' ) || ! is_user_logged_in() || ! buddyboss_theme_bb_blog_is_active() ) {
			return;
		}

		$bb_post_id = get_the_ID();
		...
		// Subscribe to topics.
		if ( function_exists( 'bb_blog_pro_is_subscriptions_enabled' ) && bb_blog_pro_is_subscriptions_enabled() ) {
			?>
			<button
				type="button"
				class="bb-blog-single-subscribe-bell js-bb-blog-subscribe-open"
				data-balloon-pos="up"
				data-balloon="<?php esc_attr_e( 'Subscribe to topics', 'buddyboss-theme' ); ?>"
				aria-label="<?php esc_attr_e( 'Subscribe to topics', 'buddyboss-theme' ); ?>"
			>
				<i class="bb-icon-l bb-icon-bell-plus"></i>
			</button>
			<?php
		}

		// Bookmark this post.
		if ( function_exists( 'bb_blog_pro_is_bookmarking_enabled' ) && bb_blog_pro_is_bookmarking_enabled() ) {
			$bb_is_bookmarked = function_exists( 'bb_blog_pro_is_bookmarked' ) && bb_blog_pro_is_bookmarked( $bb_post_id );
			?>
			<button
				type="button"
				class="bb-blog-bookmark<?php echo esc_attr( $bb_is_bookmarked ? ' is-bookmarked' : '' ); ?>"
				data-post-id="<?php echo esc_attr( $bb_post_id ); ?>"
				data-balloon-pos="up"
				data-balloon="<?php echo $bb_is_bookmarked ? esc_attr__( 'Remove bookmark', 'buddyboss-theme' ) : esc_attr__( 'Bookmark this post', 'buddyboss-theme' ); ?>"
				aria-pressed="<?php echo esc_attr( $bb_is_bookmarked ? 'true' : 'false' ); ?>"
				aria-label="<?php esc_attr_e( 'Bookmark this post', 'buddyboss-theme' ); ?>"
			>
				<i class="bb-icon-l bb-icon-bookmark"></i>
			</button>
			<?php
		}
	}
...
if ( ! function_exists( 'buddyboss_theme_bb_blog_suppress_platform_share' ) ) {
	/**
	 * Decide which share UI owns a single blog post in classic theme mode.
	 *
	 * Exactly one of the two may render:
	 *
	 * - Platform blog feature ACTIVE -- the Platform's `.bb-blog-share` row
	 *   wins, so the icons follow the "Social Links" toggles in Blog Settings
	 *   -> Page Settings. The theme's own "Share on:" box stands down (see
	 *   template-parts/content.php).
	 * - Feature INACTIVE -- the theme's box is all there is.
	 *
	 * A filter is needed rather than letting the Platform's own default decide,
	 * because that default reads the theme's `blog_share_box` Redux option --
	 * which backed the removed "Floating Social Share" setting. The lookup now
	 * returns an orphaned saved value on existing sites (suppressing the
	 * Platform row for reasons no admin can see or change) and `false` on fresh
	 * ones. This makes the choice explicit instead.
	 *
	 * ReadyLaunch is exempt: when it drives the blog surface the theme's
	 * template never outputs, so there is nothing to arbitrate.
	 *
	 * @since 2.21.0
	 *
	 * @param bool $suppress Whether to suppress the Platform share row.
	 * @param int  $post_id  Post ID.
	 *
	 * @return bool
	 */
	function buddyboss_theme_bb_blog_suppress_platform_share( $suppress, $post_id ) {
		if ( buddyboss_theme_bb_blog_rl_drives_blog() ) {
			return $suppress;
		}

		// Platform blog active: its share row -- built from the "Social Links"
		// toggles in Blog Settings -> Page Settings -- is the single source of
		// sharing, and the theme's own box stands down (see content.php).
		if ( buddyboss_theme_bb_blog_is_active() ) {
			return false;
		}

		// Feature off: the theme's own share box handles it. (Platform's footer
		// code is not even loaded in this state, so this is belt-and-braces.)
		return true;
	}

	add_filter( 'bb_blog_suppress_share_for_theme', 'buddyboss_theme_bb_blog_suppress_platform_share', 10, 2 );
}

if ( ! function_exists( 'buddyboss_theme_bb_blog_rl_drives_blog' ) ) {
	/**
	 * Whether ReadyLaunch (not this theme) is rendering the current blog screen.
	 *
	 * Mirrors the Platform's own test in `bb_blog_render_post_footer_sections()`:
	 * RL globally on, RL enabled for the blog surface, and the request actually
	 * being a blog context. When all three hold, the theme's own blog templates
	 * never output, so theme-side overrides that assume they did must stand
	 * down.
	 *
	 * @since 2.21.0
	 *
	 * @return bool
	 */
	function buddyboss_theme_bb_blog_rl_drives_blog() {
		return function_exists( 'bb_is_readylaunch_enabled' ) && bb_is_readylaunch_enabled()
			&& function_exists( 'bb_blog_rl_is_enabled' ) && bb_blog_rl_is_enabled()
			&& function_exists( 'bb_blog_is_blog_context' ) && bb_blog_is_blog_context();
	}
}

if ( ! function_exists( 'buddyboss_theme_bb_blog_is_listing' ) ) {
	/**
	 * Whether the current request renders the theme's blog listing header.
	 *
	 * Matches the two call sites of `template-parts/blog-header.php`: the posts
	 * page (index.php) and post archives (archive.php).
	 *
	 * @since 2.21.0
	 *
	 * @return bool
	 */
	function buddyboss_theme_bb_blog_is_listing() {
		return is_home() || is_category() || is_tag() || is_author() || is_date();
	}
}

if ( ! function_exists( 'buddyboss_theme_bb_blog_relocate_header_actions' ) ) {
	/**
	 * Move the blog Subscribe / Create New buttons into the listing header.
	 *
	 * Both add-ons register their button twice: once on
	 * `bb_blog_archive_header_actions` (the ReadyLaunch archive header) and
	 * once on `buddyboss_theme_before_content` as the classic-theme fallback,
	 * back when the theme had no header of its own to hook.
	 *
	 * The theme now renders `template-parts/blog-header.php`, which fires
	 * `bb_blog_archive_header_actions` itself. Left alone the fallbacks would
	 * either duplicate the buttons or -- because
	 * `bb_blog_pro_render_subscribe_button()` carries a `static $rendered`
	 * once-guard and `buddyboss_theme_before_content` fires from header.php,
	 * i.e. before the listing header -- win the race and strand the button
	 * above the content. Dropping the fallbacks leaves the header hook as the
	 * single render point, with every settings gate still enforced inside the
	 ...
	 * @return void
	 */
	function buddyboss_theme_bb_blog_relocate_header_actions() {
		if ( buddyboss_theme_bb_blog_rl_drives_blog() || ! buddyboss_theme_bb_blog_is_listing() ) {
			return;
		}

		remove_action( 'buddyboss_theme_before_content', 'bb_blog_pro_render_subscribe_button' );
		remove_action( 'buddyboss_theme_before_content', 'bb_member_blog_render_create_new_button_theme', 20 );
	}

	add_action( 'wp', 'buddyboss_theme_bb_blog_relocate_header_actions' );
}
