...
// LearnDash core populates $materials upstream in class-ld-cpt-instance.php
// before passing this template into `learndash_get_template_part()`. When
// this theme override is hit directly (e.g. from the topic permalink),
// $materials is never seeded — modules/tabs.php gates the Materials tab on
// `! empty( $materials )` and the tab silently disappears. Re-seed via the
// Topic model's Has_Materials trait (added in LD 4.6.0), mirroring how
// course.php:54 resolves `$course->get_materials()`. The trait auto-resolves
// the right `topic_materials_*` setting prefix by post type, so we don't
// duplicate sfwd-topic's key naming here.
if ( empty( $materials ) && class_exists( '\LearnDash\Core\Models\Topic' ) && isset( $post ) ) {
	$topic_model = \LearnDash\Core\Models\Topic::create_from_post( $post );
	$materials   = $topic_model->get_materials();
}
