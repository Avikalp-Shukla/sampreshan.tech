...
// this theme override is hit directly (e.g. from the lesson permalink),
...
// Lesson model's Has_Materials trait (added in LD 4.6.0), mirroring how
// course.php:54 resolves `$course->get_materials()`. Using the model keeps
// us in sync with any future changes to LD's materials pipeline.
if ( empty( $materials ) && class_exists( '\LearnDash\Core\Models\Lesson' ) && isset( $post ) ) {
	$lesson_model = \LearnDash\Core\Models\Lesson::create_from_post( $post );
	$materials    = $lesson_model->get_materials();
}
