...
class BB_Group_Activity_Topics_Setting extends BP_Group_Extension {
	...
	private function setup_actions() {
		...
		if ( function_exists( 'bb_feature_registry' ) ) {
			// Settings 2.0: Register Topics tab in Group Edit modal.
			add_action( 'bb_register_groups_meta_fields', array( $this, 'register_group_topics_tab' ), 10, 2 );
		} else {
	...
	/**
	 * Register Topics tab indicator field for Settings 2.0 Group Edit modal.
	 *
	 * The readonly field with empty value creates the "Topics" tab dynamically
	 * when the group activity topics feature is active. The tab content is
	 * handled by the GroupTopicsTab React component.
	 *
	 * @since 3.0.0
	 *
	 * @param BB_Admin_Meta_Field_Registry $registry  Meta field registry instance.
	 * @param string                       $component Component name ('groups').
	 */
	public function register_group_topics_tab( $registry, $component ) {
		$registry->register(
			$component,
			'topics_tab_indicator',
			array(
				'label'      => __( 'Topics', 'buddyboss-pro' ),
				'type'       => 'readonly',
				'tab'        => 'topics',
				'order'      => 10,
				'is_visible' => function () {
					return function_exists( 'bb_is_enabled_group_activity_topics' )
	...
	public function bb_validate_group_activity_topic_before_added( $args ) {
		if ( 'groups' === $args['item_type'] && ! empty( $args['item_id'] ) ) {
			// Topics is registered as an activity-only feature, so groups
			// component may be disabled at the site level. Without this guard
			// the calls to groups_get_group()/groups_is_user_admin() below
			// fatal with "Call to undefined function …" the moment a group-
			// scoped topic is created or validated.
