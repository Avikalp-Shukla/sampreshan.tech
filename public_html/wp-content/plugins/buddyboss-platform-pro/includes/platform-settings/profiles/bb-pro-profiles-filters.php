...
// Reset primary action when connections toggle is changed via Settings 2.0.
if ( function_exists( 'bb_register_feature' ) ) {
	add_action( 'bb_admin_save_feature_settings_after', 'bb_reset_primary_action_on_connection_toggle', 5, 3 );
}
...
function bb_reset_profile_cover_position_on_change_sizes( $old_value, $value, $option ) {
	if ( ! bb_pro_should_lock_features() && ( 'bb-pro-cover-profile-width' === $option || 'bb-pro-cover-profile-height' === $option ) ) {
		global $wpdb;
		// Single bulk DELETE instead of get_users() + loop (avoids loading all user objects).
		$wpdb->delete( $wpdb->usermeta, array( 'meta_key' => 'bp_cover_position' ) ); // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
	}
}
...
/**
 * Reset primary action in member directories when connection toggle changes via Settings 2.0.
 *
 * When `bb_enable_member_connections` is toggled OFF in Settings 2.0, the legacy
 * `bp_core_install` hook does not fire. This handler covers that gap by resetting
 * the primary action to empty if it was 'connect'.
 *
 * @since 3.0.0
 *
 ...
 */
function bb_reset_primary_action_on_connection_toggle( $feature_id, $settings, $saved ) {
	...
	// Only act when connections are being disabled.
	if ( $enable ) {
		return;
	}
	...
	if ( 'connect' === $primary_action ) {
		bp_update_option( 'bb-pro-member-profile-primary-action', '' );
	}
}
