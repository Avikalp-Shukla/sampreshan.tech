...
function bbp_pro_version_updater() {
	...
	if ( bbp_pro_is_install() ) {
		...
		// Settings 2.0: migrate OneSignal, Zoom, and Pusher integration feature toggles.
		if ( $raw_db_version < 335 ) {
			bbp_pro_update_to_2_13_10();
		}
...
function bb_pro_sso_remove_attachments_callback() {
	...
	// Build the SQL with a dynamic LIKE clause. Every %s/%d placeholder is
	// matched with a corresponding value in $query_arguments before we hand
	// it to $wpdb->prepare(), so the query is fully parameterised even
	// though the SQL itself is assembled at runtime.
	...
	// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- $sql is built from hardcoded LIKE tokens and all values are passed through $wpdb->prepare() below.
	$results = $wpdb->get_col( $wpdb->prepare( $sql, $query_arguments ) );

	if ( empty( $results ) ) {
		return;
	}

	// Cast every ID and build %d placeholders for the IN clause so the
	// query is fully prepared (previously interpolated a comma-joined string).
	$post_ids          = array_map( 'intval', $results );
	$post_placeholders = implode( ',', array_fill( 0, count( $post_ids ), '%d' ) );
	$blog_user_avatar  = $wpdb->get_blog_prefix( $blog_id ) . 'user_avatar';

	// Delete user meta in a single query.
	// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $post_placeholders is a hardcoded list of %d tokens built from array_fill().
	$wpdb->query(
		$wpdb->prepare(
			"DELETE um FROM {$wpdb->usermeta} um
				...
				AND ( um.meta_key = %s OR um.meta_key = 'bb_sso_user_avatar_md5' )",
			array_merge( $post_ids, array( $blog_user_avatar ) )
		)
	);
...
/**
 * Settings 2.0 migration: enable OneSignal, Zoom, and Pusher integration feature
 * toggles based on existing connected state.
 *
 * In legacy, these integrations had no main feature toggle — they were always
 * available once configured. Settings 2.0 introduces feature cards with on/off
 * toggles. This migration ensures that sites with existing connections have
 * their toggles enabled after upgrade.
 *
 * @since 3.0.0
 */
function bbp_pro_update_to_2_13_10() {

	$active_features = bp_get_option( 'bb-active-features', array() );
	$needs_update    = false;

	// OneSignal: enable if previously connected or configured.
	$onesignal_settings = bp_get_option( 'bb-onesignal', array() );
	$os_connected       = ! empty( $onesignal_settings['is_connected'] );
	$os_credentials     = ! empty( $onesignal_settings['app_id'] ) && ! empty( $onesignal_settings['rest_api_key'] );

	if ( $os_connected || $os_credentials ) {
		$active_features['onesignal'] = 1;
		$needs_update                 = true;
	}

	// Zoom: enable if S2S/SDK connected or has any credentials.
	$zoom_settings = bp_get_option( 'bb-zoom', array() );
	$s2s_connected = ! empty( $zoom_settings['zoom_is_connected'] );
	$sdk_connected = ! empty( $zoom_settings['zoom_sdk_is_connected'] );
	$has_s2s_creds = ! empty( $zoom_settings['s2s-account-id'] ) && ! empty( $zoom_settings['s2s-client-id'] ) && ! empty( $zoom_settings['s2s-client-secret'] );
	$has_sdk_creds = ! empty( $zoom_settings['meeting-sdk-client-id'] ) && ! empty( $zoom_settings['meeting-sdk-client-secret'] );

	if ( $s2s_connected || $sdk_connected || $has_s2s_creds || $has_sdk_creds ) {
		$active_features['zoom'] = 1;
		$needs_update            = true;
	}

	// Pusher: enable only when the integration was previously validated and
	// marked connected by bb_pusher_credential_validate(). Stale credentials
	// from an aborted Connect flow should NOT auto-enable the feature.
	if ( (bool) bp_get_option( 'bb-pusher-enabled', false ) ) {
		$active_features['pusher'] = 1;
		$needs_update              = true;
	}
