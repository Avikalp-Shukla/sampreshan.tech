	...
	class BB_Reactions_Background_Process extends BB_Background_Process {
		...
		protected function task( $callback ) {
			...
			if ( is_callable( $callback ) ) {
				if ( function_exists( 'bb_error_log' ) ) {
					// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- bb_error_log is gated behind BB_DEBUG_LOG.
					bb_error_log( sprintf( 'Running %s callback', json_encode( $callback ) ) );
				} elseif ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Debug-only fallback when bb_error_log is unavailable.
		...
		protected function completed() {
			...
			if ( $is_reaction_migration ) {
				bp_update_option(
					'bb_pro_reaction_migration_completed',
					array(
						'success'     => 'yes',
		...
		public function delete_all_batches() {
			...
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Table name is a trusted class constant.
			$wpdb->query(
				$wpdb->prepare(
					"DELETE FROM {$table} WHERE `blog_id` = %d AND `group` = 'bb_pro_migrate_reactions' AND `secondary_data_id` != 'delete'",
					$blog_id
				)
			);
