...
class BB_Reactions_Picker {
	...
	public function enqueue_scripts() {
		...
		if ( ! function_exists( 'bb_register_feature' ) && ( 'bp-settings' !== $page || 'bp-reactions' !== $current_tab ) ) {
			return;
		} elseif ( function_exists( 'bb_register_feature' ) && 'bb-settings' !== $page ) {
			return;
		}
		...
		wp_enqueue_script( 'bb-emotion-picker', bb_reaction_url( '/assets/js/admin/bb-emotion-picker' . $min . '.js' ), array( 'wp-util', 'wp-color-picker' ), bb_platform_pro()->version, true );
	...
	public function delete_custom_icon() {
		...
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( wp_unslash( $_POST['nonce'] ), 'bbpro-delete-custom-icon' ) ) { //phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			...
			if ( 'delete_custom_icon' === $task ) {
				...
				$all_emotions      = $reaction_instance ? $reaction_instance->bb_get_reactions( 'emotions', false ) : array();
	...
	public function bb_icons_dir() {
		return trailingslashit( bb_platform_pro()->reactions_dir ) . 'assets/icons/bb-icons/';
	}
	...
	public function render_emoji_category_list() {
		...
		<select class="bbpro-icon-category-filter-select" id="emoji-categories">
			...
			foreach ( $emoji_categories as $emoji_category ) {
				printf( '<option value="%1$s">%2$s</option>', esc_attr( $emoji_category ), esc_html( $emoji_category ) );
			}
			?>
		</select>
		<?php
