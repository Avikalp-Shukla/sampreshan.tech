			...
			$( document ).on(
				...
				function () {
					...
					if ( 'reactionButton' === self.editorMode ) {
						...
						isReactAdmin = window.bbReactEmotionCallbacks &&
						               typeof window.bbReactEmotionCallbacks.updateReactionButton === 'function';

						if ( isReactAdmin ) {
							// React context: Call the callback to update React state
							window.bbReactEmotionCallbacks.updateReactionButton( self.iconPreviewData.icon );
						} else {
							// Old admin context: Use original jQuery DOM manipulation
					...
					if ( self.currentEventHandler !== null ) {
						// Check if we're in React admin context
						isReactAdmin = window.bbReactEmotionCallbacks &&
						               typeof window.bbReactEmotionCallbacks.updateEmotion === 'function';

						if ( isReactAdmin ) {
							// React context: Call the callback instead of DOM manipulation
							var isEdit = self.currentEventHandler.hasClass( 'bb_emotions_edit' );
							window.bbReactEmotionCallbacks.updateEmotion( self.iconPreviewData, isEdit );
						} else {
							// Old admin context: Use original jQuery DOM manipulation
