...
function bb_pro_get_reactions( $mode = 'likes', $is_active = true ) {
	...
	return apply_filters( 'bb_pro_get_reactions', $reaction_instance->bb_get_reactions( $mode, $is_active ) );
}
...
function bb_admin_setting_reaction_notice_field_callback( $print = true ) {
			...
			if (
				...
				if ( 'like_to_emotions_action' === $migration_data['action'] ) {
					printf(
						...
						wp_kses(
							sprintf(
								/* translators: 1: Number and name of emotions (e.g. "10 Likes"), 2: Target emotion name (e.g. "Love"). */
								__( '%1$s were successfully converted to the %2$s emotion.', 'buddyboss-pro' ),
								sprintf(
									'<strong>' . esc_html( bp_core_number_format( $migration_data['total_reactions'] ) ) . ' %s</strong>',
									esc_html( $migration_data['from_emotions_name'] )
								),
								'<strong>' . esc_html( $migration_data['to_emotions_name'] ) . '</strong>' // @todo Update emotion name.
							),
							array(
								'strong' => array(),
							)
					...
					printf(
						...
						wp_kses(
							sprintf(
								/* translators: 1: Number of reactions (e.g. "<strong>10</strong>"), 2: Target emotion name. */
								__( '%1$s <strong>reactions</strong> were successfully converted %2$s.', 'buddyboss-pro' ),
								'<strong>' . esc_html( bp_core_number_format( $migration_data['total_reactions'] ) ) . '</strong>',
								'<strong>' . esc_html( $migration_data['to_emotions_name'] ) . '</strong>'
							),
							array(
								'strong' => array(),
							)
				...
				printf(
					...
					esc_html(
						sprintf(
							/* translators: 1: Number of converted reactions, 2: Total reactions, 3: Percentage string (e.g. "(50%)"). */
					...
					esc_html__( 'This action is being performed in the background, but may take some time based on the amount of data.', 'buddyboss-pro' ),
					esc_html__( 'Recheck status', 'buddyboss-pro' ),
					esc_html__( 'Stop', 'buddyboss-pro' )
				);
			} else {
				...
				if (
					...
					if ( 'like_to_emotions_action' === $migration_data['action'] ) {
						$migration_notice = sprintf(
							...
							esc_html__( 'Do you want to convert your existing Likes to an Emotion?', 'buddyboss-pro' ),
							wp_kses(
								sprintf(
									/* translators: %s: Number of Likes as a span element. */
									__( 'You have %s Likes previously submitted on your site which can be converted to an Emotion.', 'buddyboss-pro' ),
									'<span class="reaction-notice-count">' . esc_html( bp_core_number_format( $migration_data['total_reactions'] ) ) . '</span>'
								),
								array(
						...
						$migration_notice = sprintf(
							'<p><strong>%1$s</strong></p><p>%2$s</p>',
							esc_html__( 'Do you want to convert your existing reactions to Likes?', 'buddyboss-pro' ),
							wp_kses(
								sprintf(
									/* translators: %s: Number of reactions as a span element. */
					...
					if ( ! empty( $migration_notice ) ) {
						printf(
							...
							esc_html__( 'Start conversion', 'buddyboss-pro' ),
							esc_html__( 'Do later', 'buddyboss-pro' )
						);
					}
				}
			}
	...
	if ( $print ) {
		echo wp_kses( $html, bb_pro_reaction_wizard_allowed_tags() );
	} else {
		return $html;
	}
}
...
function bb_pro_reaction_get_no_data_screen( $reaction_mode ) {
	...
	printf(
		...
		esc_html__( 'You have no reactions to convert', 'buddyboss-pro' )
	);

	if ( 'emotions' === $reaction_mode ) {
		printf(
			'<p class="text-center">%s</p>',
			esc_html__( 'Unable to find any existing reactions to convert to an Emotion.', 'buddyboss-pro' )
		);
	} elseif ( 'likes' === $reaction_mode ) {
		printf(
			'<p class="text-center">%s</p>',
			esc_html__( 'Unable to find any existing reactions to convert to Likes.', 'buddyboss-pro' )
		);
	}
...
function bb_pro_reaction_get_migration_wizard( $args = array() ) {
	...
	$likes['count'] = $reaction_instance->bb_get_user_reactions_count( array( 'reaction_id' => $likes['id'] ) );
	...
	if ( ! empty( $emotions ) ) {
		foreach ( $emotions as $reaction_key => $reaction ) {
			// Fetch total count.
			$total_count                        = $reaction_instance->bb_get_user_reactions_count( array( 'reaction_id' => $reaction['id'] ) );
	...
	switch ( $r['action_type'] ) {
		case 'footer_wizard':
			if ( 0 < $total_reaction_count && 'emotions' === $r['reaction_mode'] ) {
				ob_start();
				?>
				<p><?php echo esc_html__( 'This action will convert reactions previously submitted by members on your site to an Emotion of your choice. Reactions not selected can be converted at any point in the future using this migration wizard.', 'buddyboss-pro' ); ?></p>
				<p><strong><?php echo esc_html__( 'Which reactions do you want to convert?', 'buddyboss-pro' ); ?></strong></p>
				<div class="migration_emotion_list">
					...
					if ( 0 < (int) $likes['count'] ) {
						...
						<p>
							<label for="migrate_like_reaction">
								...
								<?php echo esc_html__( 'Likes', 'buddyboss-pro' ); ?> (<?php echo esc_html( bp_core_number_format( $likes['count'] ) ); ?>)
							</label>
						</p>
						<?php
					}
					...
					<p>
						<label for="migrate_all_emotions">
							...
							<?php echo esc_html__( 'Emotions', 'buddyboss-pro' ); ?> (<?php echo esc_html( bp_core_number_format( $total_emotion_count ) ); ?>)
						</label>
					</p>
					<ul>
						<?php
						if ( ! empty( $emotions ) ) {
							foreach ( $emotions as $reaction ) {
								printf(
									...
									esc_attr( $reaction['count'] ),
									esc_html( bp_core_number_format( $reaction['count'] ) )
								);
							}
						}
						?>
				...
				<p><strong><?php echo esc_html__( 'Which Emotion would you like to convert your reactions to?', 'buddyboss-pro' ); ?></strong></p>
				...
				<p>
					...
					printf(
						wp_kses(
							/* translators: 1: Reaction count span with label, 2: Target emotion label span. */
						...
						wp_kses(
							sprintf(
								...
								esc_html__( 'reactions', 'buddyboss-pro' )
							),
							array(
								'strong' => array(),
								'span'   => array( 'class' => true ),
				...
				<ul>
					<li><?php echo esc_html__( 'The new reactions will be immediately visible on your site after being converted', 'buddyboss-pro' ); ?></li>
					<li><?php echo esc_html__( 'Depending on the amount of data to convert, the migration may take a while', 'buddyboss-pro' ); ?></li>
					<li><?php echo esc_html__( 'You will be unable to edit reactions while the conversion is in progress', 'buddyboss-pro' ); ?></li>
					<li><?php echo esc_html__( 'This action cannot be undone, but you can convert reactions to another reaction in the future', 'buddyboss-pro' ); ?></li>
					<li><?php echo esc_html__( 'We recommend backing up your site before migrating and performing this action during an off-peak period', 'buddyboss-pro' ); ?></li>
				</ul>
				<p><?php echo esc_html__( 'Do you want to start the conversion now?', 'buddyboss-pro' ); ?></p>
				<?php
				...
				ob_start();
				?>
				<p><?php echo esc_html__( 'This action will convert reactions previously submitted by members on your site to Likes. Reactions not selected can be converted at any point in the future using this migration wizard.', 'buddyboss-pro' ); ?></p>
				<p><strong><?php echo esc_html__( 'Which reactions do you want to convert to Likes?', 'buddyboss-pro' ); ?></strong></p>
				<div class="migration_emotion_list">
					<p>
						<label for="migrate_all_emotions">
							<input type="checkbox" class="migrate_emotion_input" name="from_all_emotions" id="migrate_all_emotions">
							<?php echo esc_html__( 'All emotions', 'buddyboss-pro' ); ?> (<?php echo esc_html( bp_core_number_format( $total_emotion_count ) ); ?>)
						</label>
					</p>
					<ul>
						<?php
				...
				<p>
					...
					printf(
						wp_kses(
							/* translators: 1: Reaction count span with label, 2: Target label span. */
				...
				<ul>
					<li><?php echo esc_html__( 'The new Likes will be immediately visible on your site after being converted', 'buddyboss-pro' ); ?></li>
					<li><?php echo esc_html__( 'Depending on the amount of data to convert, the migration may take a while', 'buddyboss-pro' ); ?></li>
					<li><?php echo esc_html__( 'You will be unable to edit reactions while the conversion is in progress', 'buddyboss-pro' ); ?></li>
					<li><?php echo esc_html__( 'This action cannot be undone, but you can convert Likes back to other reactions in the future', 'buddyboss-pro' ); ?></li>
					<li><?php echo esc_html__( 'We recommend backing up your site before migrating and performing this action during an off-peak period', 'buddyboss-pro' ); ?></li>
				</ul>
				<p><?php echo esc_html__( 'Do you want to start the conversion now?', 'buddyboss-pro' ); ?></p>
				<?php
		...
		case 'switch_wizard':
			if ( 'emotions' === $r['reaction_mode'] ) {
				ob_start();
				?>
				<p><?php echo esc_html__( 'This action will convert reactions previously submitted by members on your site to Likes. Reactions not selected can be converted at any point in the future using the migration wizard.', 'buddyboss-pro' ); ?></p>
				<p><strong><?php echo esc_html__( 'Which reactions do you want to convert to Likes?', 'buddyboss-pro' ); ?></strong></p>
				<div class="migration_emotion_list">
					<p>
						<label for="migrate_all_emotions">
				...
				<p>
					...
					printf(
						wp_kses(
							/* translators: 1: Reaction count span with label, 2: Target "Likes" label span. */
						...
						sprintf(
							'<strong>%s</strong>',
							esc_html__( 'Likes', 'buddyboss-pro' )
						)
					);
					?>
				</p>
				...
				<p>
					...
					printf(
						wp_kses(
							/* translators: %s: Number and "Likes" label wrapped in strong tags. */
							__( 'This action will convert the %s previously submitted by members on your site to an Emotion of your choice. You can perform this action at any point in the future using this migration wizard.', 'buddyboss-pro' ),
							array(
								'strong' => array(),
							)
						),
						sprintf(
							...
							esc_html( bp_core_number_format( $likes['count'] ) ),
							esc_html__( 'Likes', 'buddyboss-pro' )
						)
					);
					?>
				...
				<p><?php echo esc_html__( 'Which Emotion would you like to convert your Likes to?', 'buddyboss-pro' ); ?></p>
				<select name="to_reactions" id="migration_emotion_select">
					<option value=""><?php echo esc_html__( 'Select an Emotion', 'buddyboss-pro' ); ?></option>
					<?php
					if ( ! empty( $emotions ) ) {
						foreach ( $emotions as $emotion ) {
							printf(
				...
				<ul>
					...
					<li><?php echo esc_html__( 'Depending on the amount of data to convert, the migration may take a while', 'buddyboss-pro' ); ?></li>
					<li><?php echo esc_html__( 'You will be unable to edit reactions while the conversion is in progress', 'buddyboss-pro' ); ?></li>
					<li><?php echo esc_html__( 'This action cannot be undone, but you can convert reactions back to Likes in the future', 'buddyboss-pro' ); ?></li>
					<li><?php echo esc_html__( 'We recommend backing up your site before migrating and performing this action during an off-peak period', 'buddyboss-pro' ); ?></li>
				</ul>
				<p><?php echo esc_html__( 'Do you want to start the conversion now?', 'buddyboss-pro' ); ?></p>
				<?php
...
function bb_pro_migrate_reactions_callback( $migrate_from_ids, $migrate_to_id, $job_from ) {
	...
	$bb_user_reaction_tbl = $reaction_instance::$user_reaction_table;

	// Use placeholders for IN clauses to avoid SQL injection (proper parameter binding).
	$reaction_id_placeholders = implode( ',', array_fill( 0, count( $migrate_from_ids ), '%d' ) );
	// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Table name is a trusted class constant; placeholders are safely generated via array_fill.
	$results = $wpdb->get_results(
		$wpdb->prepare(
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			"SELECT id, user_id, reaction_id, item_type, item_id FROM {$bb_user_reaction_tbl} WHERE `reaction_id` IN ({$reaction_id_placeholders}) ORDER BY item_id, item_type LIMIT %d",
			array_merge( $migrate_from_ids, array( $limit ) )
		),
		ARRAY_A
	);
	...
	$reaction_index_ids = array_map( 'absint', array_column( $results, 'id' ) );
	$id_placeholders    = implode( ',', array_fill( 0, count( $reaction_index_ids ), '%d' ) );

	if ( 'delete' !== $migrate_to_id ) {
		$update_reaction_placeholders = implode( ',', array_fill( 0, count( $migrate_from_ids ), '%d' ) );
		// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Table name is a trusted class constant; placeholders are safely generated via array_fill.
		$wpdb->query(
			$wpdb->prepare(
				// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				"UPDATE {$bb_user_reaction_tbl} SET `reaction_id` = %d WHERE `reaction_id` IN ({$update_reaction_placeholders}) AND id IN ({$id_placeholders})",
				array_merge( array( $migrate_to_id ), $migrate_from_ids, $reaction_index_ids )
			)
		);
		...
		$wpdb->query(
			$wpdb->prepare(
				// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				"DELETE FROM {$bb_user_reaction_tbl} WHERE `id` IN ({$id_placeholders})",
				$reaction_index_ids
			)
		);
	...
	if ( ! empty( $old_data ) ) {
		...
		foreach ( $old_data as $reaction ) {
			$reaction_instance->bb_prepare_reaction_summary_data( $reaction );
		}
	}
 ...
 * @since 2.4.50
 *
 * @param bool $reset_cache Optional. Set to true to reset the static cache. Default false.
 *
 * @return string Migration status: 'completed', 'inprogress', or empty string.
 */
function bb_pro_reaction_get_migration_status( $reset_cache = false ) {
	static $cache = null;

	// Reset cache when explicitly requested (e.g., after deleting migration).
	if ( $reset_cache ) {
		$cache = null;
	}
...
function bb_pro_reaction_add_migration_action( $args = array() ) {
	...
	if ( ! empty( $r['action'] ) ) {
		// Clear any previous completed migration status so the new migration shows as pending.
		bp_delete_option( 'bb_pro_reaction_migration_completed' );
...
/**
 * Function to update migration action.
 *
 * @since 2.4.50
 *
 * @param array $args Data of migration.
...
function bb_pro_reaction_delete_migration() {
	...
	// Also clear any previous completed migration status so new migration shows as pending.
	bp_delete_option( 'bb_pro_reaction_migration_completed' );
	// Reset the status cache so subsequent calls get fresh status.
	bb_pro_reaction_get_migration_status( true );
}

/**
 * Get allowed HTML tags for reaction migration wizard.
 *
 * @since 3.0.0
 *
 * @return array Allowed HTML tags and attributes.
 */
function bb_pro_reaction_wizard_allowed_tags() {
	return array(
		'p'      => array(
			'class' => true,
		),
