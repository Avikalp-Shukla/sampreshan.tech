...
class BP_Zoom_Group {
	...
	/**
	 * Register Zoom group meta fields for the Settings 2.0 edit modal.
	 *
	 * @since 3.0.0
	 *
	...
	public function register_group_meta_fields( $registry, $component ) {
		...
		// Enable Zoom toggle (checkbox).
		$registry->register(
			$component,
			'zoom_enable',
			array(
				'label'             => __( 'Connect this group to Zoom', 'buddyboss-pro' ),
				'description'       => __( 'Create and sync Zoom meetings and webinars directly within this group.', 'buddyboss-pro' ),
				'type'              => 'checkbox',
				'tab'               => 'integrations',
				'order'             => 400,
				'save_phase'        => 'after',
				'get_value'         => function ( $group ) {
					return (bool) groups_get_groupmeta( $group->id, 'bp-group-zoom', true ) ? '1' : '0';
				},
				'save_value'        => function ( $group, $value ) {
					$edit_zoom = ! empty( $value ) && '0' !== $value;
					groups_update_groupmeta( $group->id, 'bp-group-zoom', $edit_zoom );
				},
				'sanitize_callback' => 'sanitize_text_field',
				'is_visible'        => function () {
					return bp_zoom_is_zoom_groups_enabled();
				},
			)
		);

		// Zoom connection type (group's own app vs site app).
		$registry->register(
			$component,
			'zoom_connection_type',
			array(
				'label'             => __( 'How should this group be connected to Zoom?', 'buddyboss-pro' ),
				'type'              => 'radio',
				'tab'               => 'integrations',
				'order'             => 405,
				'save_phase'        => 'after',
				...
				'get_value'         => function ( $group ) {
					if ( function_exists( 'bb_zoom_group_get_connection_type' ) ) {
						return bb_zoom_group_get_connection_type( $group->id );
					}
					$type = groups_get_groupmeta( $group->id, 'bp-group-zoom-connection-type', true );
					return ! empty( $type ) ? $type : 'group';
				},
				'get_options'       => function () {
					$options = array(
						array(
							'label' => __( 'Allow group organizers to connect their own Zoom app', 'buddyboss-pro' ),
							'value' => 'group',
						),
					);

					// Only show 'site' option if site-level S2S is connected.
					if ( function_exists( 'bb_zoom_is_s2s_connected' ) && bb_zoom_is_s2s_connected() ) {
						$options[] = array(
							'label' => __( "Use this site's Zoom app", 'buddyboss-pro' ),
							'value' => 'site',
						);
					}
				...
				'save_value'        => function ( $group, $value ) {
					$allowed = array( 'group', 'site' );

					// Validate: if site S2S not connected, fallback to group.
					if ( 'site' === $value && function_exists( 'bb_zoom_is_s2s_connected' ) && ! bb_zoom_is_s2s_connected() ) {
						$value = 'group';
					}

					if ( in_array( $value, $allowed, true ) ) {
						groups_update_groupmeta( $group->id, 'bp-group-zoom-connection-type', $value );
					}
				},
		...
		// Zoom manager permission (who can host).
		$registry->register(
			$component,
			'zoom_manager',
			array(
				'label'             => __( 'Which group members can create, edit and delete Zoom meetings?', 'buddyboss-pro' ),
				'type'              => 'radio',
				'tab'               => 'integrations',
				'order'             => 410,
				'save_phase'        => 'after',
				...
				'get_value'         => function ( $group ) {
					$manager = groups_get_groupmeta( $group->id, 'bp-group-zoom-manager', true );
					return ! empty( $manager ) ? $manager : 'admins';
				},
				'get_options'       => function () {
					return array(
						array(
							'label' => __( 'Organizers Only', 'buddyboss-pro' ),
							'value' => 'admins',
						),
						array(
							'label' => __( 'Organizers and Moderators Only', 'buddyboss-pro' ),
							'value' => 'mods',
						),
						array(
							'label' => __( 'All Group Members', 'buddyboss-pro' ),
							'value' => 'members',
						),
					);
				},
				'save_value'        => function ( $group, $value ) {
					$allowed = array( 'admins', 'mods', 'members' );
					if ( in_array( $value, $allowed, true ) ) {
						groups_update_groupmeta( $group->id, 'bp-group-zoom-manager', $value );
					}
				},
				'sanitize_callback' => 'sanitize_text_field',
				'is_visible'        => function () {
