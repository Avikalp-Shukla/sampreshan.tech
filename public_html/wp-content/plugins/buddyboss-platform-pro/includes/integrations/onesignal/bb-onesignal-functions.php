...
function bb_onesignal_update_app_details() {
	...
	if ( ! empty( $auth_key ) && ! empty( $app_id ) ) {
		...
		// Handle connection failure (DNS, SSL, timeout).
		if ( is_wp_error( $request ) ) {
			$settings['errors']['connection_error'] = sprintf(
				...
				sanitize_text_field( $request->get_error_message() )
			);
		...
		if ( 200 === $response_code && ! empty( $response ) ) {
			if ( ! empty( $web_url['scheme'] ) && 'http' === $web_url['scheme'] && empty( $response['chrome_web_sub_domain'] ) ) {
				$settings['warnings']['only_ssl'] = sprintf(
					...
					'<strong>' . esc_html( $response['name'] ) . ' </strong>',
			...
			if ( ! empty( $response['errors'] ) ) {
				$response_errors = is_array( $response['errors'] )
				? implode( ', ', array_map( 'sanitize_text_field', $response['errors'] ) )
				: sanitize_text_field( $response['errors'] );
			}
