	...
	function ( exports, $ ) {
		...
		bb.SSO_Admin = {
			...
			bbSSOConnectionStep: function ( event ) {
				...
				// Settings 2.0 puts Cancel leftmost (matches the verify-popup
				// pattern used by Pusher/Zoom/etc.). Older Platform installs —
				// where bbSSOAdminVars.is_settings_v2 is false — keep the
				// legacy [verify][submit][cancel] order so we don't change
				// any UI on sites that haven't updated Platform yet.
				var isSettingsV2 = !! ( window.bbSSOAdminVars && window.bbSSOAdminVars.is_settings_v2 );
				...
				if ( 'not-configured' !== hiddenAttr.state ) {
					...
					if ( $( ssoPopup ).find( '.bb-hello-sso #sso_submit' ).length > 0 ) {
						// Submit already exists — verify lands just before it
						// in BOTH layouts. In Settings 2.0 the surrounding order
						// is [cancel][verify][submit]; in legacy it's
						// [verify][submit][cancel]. The relative position of
						// verify-vs-submit is identical, so this line is shared.
						$( ssoPopup ).find( '.bb-hello-sso #sso_submit' ).before( buttonHtmlElem );
					} else if ( isSettingsV2 ) {
						$( ssoPopup ).find( '.bb-hello-sso #sso_cancel' ).after( buttonHtmlElem );
					} else {
				...
				if ( 'not-configured' === hiddenAttr.state ) {
					...
					if ( isSettingsV2 ) {
						$( ssoPopup ).find( '.bb-hello-sso #sso_cancel' ).after( submitButton );
					} else {
			...
			enableSaveChangesButton: function ( event ) {
				...
				// See note in bbSSOConnectionStep above for the Settings 2.0
				// vs legacy ordering rationale.
				var isSettingsV2Live = !! ( window.bbSSOAdminVars && window.bbSSOAdminVars.is_settings_v2 );
				...
				if ( - 1 !== jQuery.inArray( false, allowSubmit ) ) {
					...
					if ( ! verifyButtonElem.length ) {
						...
						if ( ssoSubmitElem.length > 0 ) {
							// Verify-before-submit is consistent across both
							// layouts; relative order is the same.
							ssoSubmitElem.before( buttonHtmlElem );
						} else if ( isSettingsV2Live ) {
							cancelElem.after( buttonHtmlElem );
						} else {
