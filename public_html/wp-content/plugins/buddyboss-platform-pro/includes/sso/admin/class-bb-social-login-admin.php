...
class BB_Social_Login_Admin {
	...
	public static function bb_sso_save_settings() {
		if ( ! bp_current_user_can( 'bp_moderate' ) ) {
			wp_send_json_error( __( 'Permission denied.', 'buddyboss-pro' ) );
		}
		...
		if ( isset( BB_SSO::$providers[ $provider_id ] ) ) {
			...
			if ( 'google' === $provider_id ) {
				...
				$posed_client_id     = isset( $_POST['client_id'] ) ? sanitize_text_field( wp_unslash( $_POST['client_id'] ) ) : '';
				$posed_client_secret = isset( $_POST['client_secret'] ) ? sanitize_text_field( wp_unslash( $_POST['client_secret'] ) ) : '';
			...
			if ( BB_SSO::$providers[ $provider_id ]->settings->update( wp_unslash( $_POST ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified above.
	...
	public static function bb_save_sso_orders() {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Nonce verified above. Sanitized via map_deep.
		$raw_ordering = isset( $_POST['ordering'] ) ? map_deep( wp_unslash( $_POST['ordering'] ), 'sanitize_text_field' ) : '';
		if ( ! empty( $raw_ordering ) ) {
			if (
