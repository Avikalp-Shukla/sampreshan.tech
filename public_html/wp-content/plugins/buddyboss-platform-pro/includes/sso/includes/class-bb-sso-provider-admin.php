		...
		foreach ( $posted_data as $key => $value ) {
			...
			switch ( $key ) {
				...
				case 'oauth_redirect_url':
					// Restrict scheme to http/https — OAuth callbacks must be one of
					// these. esc_url_raw enforces the allowlist and strips
					// `javascript:` / `data:` / etc. We do NOT use
					// wp_http_validate_url here because it also rejects loopback
					// (127.x), private (10.x / 172.16-31.x / 192.168.x) and 0.x
					// hosts and runs a blocking DNS lookup — both legitimate for
					// self-hosted Keycloak / Auth0, intranet OAuth, and
					// dev/staging on localhost.
					$new_data[ $key ] = (string) esc_url_raw( (string) $value, array( 'http', 'https' ) );
					break;
			}
		}
