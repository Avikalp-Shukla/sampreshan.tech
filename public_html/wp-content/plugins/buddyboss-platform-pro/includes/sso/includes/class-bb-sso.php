...
class BB_SSO {
	...
	public static function bb_sso_scripts( $hook ) {
		$is_legacy_settings = false !== strpos( $hook, 'bp-settings' );
		$is_settings_v2     = function_exists( 'bb_register_feature' ) && false !== strpos( $hook, 'bb-settings' );

		if ( is_admin() && ! $is_legacy_settings && ! $is_settings_v2 ) {
			return;
		}
	...
	public static function bb_sso_admin_enqueue( $hook ) {
		// Legacy Settings 1.0 (bp-settings) OR Settings 2.0 (bb-settings).
		$is_legacy_settings = false !== strpos( $hook, 'bp-settings' );
		$is_settings_v2     = function_exists( 'bb_register_feature' ) && false !== strpos( $hook, 'bb-settings' );

		if ( ! $is_legacy_settings && ! $is_settings_v2 ) {
			return;
		}
		$min     = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '' : '.min';
		$rtl_css = is_rtl() ? '-rtl' : '';
		...
		wp_localize_script(
			...
			array(
				...
				// Gate Settings 2.0-only modal tweaks (Cancel-leftmost button
				// order, primary verify CTA) so older Platform installs without
				// the Settings 2.0 React admin keep their legacy modal layout.
				'is_settings_v2' => function_exists( 'bb_register_feature' ),
			)
		);
