...
class WPCode_Smart_Tags_Pro extends WPCode_Smart_Tags {
	...
	public function tag_value_wc_product_short_description() {
		...
		// Return sanitized description, this is substituted without HTML escaping.
  ...
  /**
	...
	public function tag_value_custom_field( $parameters = array() ) {
		...
		// Never expose protected (_-prefixed) meta keys to the front end.
		if ( is_protected_meta( $parameters['custom_field'], 'post' ) ) {
			return '';
		}
		...
		// If we found a meta, let's return it. Escaping happens centrally in replace_tag_occurrences().
		if ( ! empty( $meta ) && is_scalar( $meta ) ) {
			return (string) $meta;
		}
	...
	public function replace_tags_in_snippet( $code, $snippet = null, $replace_old_format = false ) {
		...
		foreach ( $smart_tags as $smart_tag_in_code => $smart_tag_key ) {
			...
			$code = $this->replace_tag_occurrences( $code, $smart_tag_in_code, $function, $parameters );
		}
		...
		if ( $replace_old_format ) {
			...
			foreach ( $tags as $tag => $function ) {
				$code = $this->replace_tag_occurrences( $code, '[' . $tag . ']', $function );
			}
		}
	...
	/**
	 * Replace every occurrence of a smart tag placeholder with its escaped value.
	 *
	 * Each occurrence is escaped for the context it sits in. Values are HTML-escaped
	 * by default: esc_html() encodes < > & " ', which neutralises attribute/script
	 * breakouts. Tags that intentionally return HTML markup (see get_html_tag_methods())
	 * keep their markup only in HTML body context - their own wp_kses_post() output still
	 * contains quotes, so inside a tag's attributes or a <script> block they are
	 * HTML-escaped like every other tag.
	 *
	 * @param string   $code        The snippet code to replace the placeholder in.
	 * @param string   $placeholder The exact placeholder string as found in the code.
	 * @param callable $function    The value-producing callback registered for the tag.
	 * @param array    $parameters  Parameters to pass to the callback.
	 *
	 * @return string
	 */
	protected function replace_tag_occurrences( $code, $placeholder, $function, $parameters = array() ) {
		if ( false === strpos( $code, $placeholder ) ) {
			return $code;
		}

		$raw_value = (string) call_user_func( $function, $parameters );
		$is_html   = $this->is_html_tag_function( $function );
		$length    = strlen( $placeholder );
		$offset    = 0;

		while ( false !== ( $position = strpos( $code, $placeholder, $offset ) ) ) {
			if ( $is_html && 'html' === $this->get_tag_context( $code, $position ) ) {
				$value = $raw_value;
			} else {
				$value = esc_html( $raw_value );
			}
			$code   = substr_replace( $code, $value, $position, $length );
	...
	/**
	 * Determine the output context of a position in the snippet code.
	 *
	 * @param string $code     The snippet code.
	 * @param int    $position The position of the smart tag occurrence.
	 *
	 * @return string One of 'attribute' (inside a tag's attributes), 'script' (inside a <script> block) or 'html'.
	 */
	protected function get_tag_context( $code, $position ) {
		$before = substr( $code, 0, $position );

		// Inside a tag's attributes: the last "<" is not followed by a closing ">".
		$last_bracket_open  = strrpos( $before, '<' );
		$last_bracket_close = strrpos( $before, '>' );
		if ( false !== $last_bracket_open && ( false === $last_bracket_close || $last_bracket_close < $last_bracket_open ) ) {
			return 'attribute';
		}

		// Inside a script block: the last opening <script> tag is not closed yet.
		$last_script_open = strripos( $before, '<script' );
		if ( false !== $last_script_open ) {
			$last_script_close = strripos( $before, '</script' );
			if ( false === $last_script_close || $last_script_close < $last_script_open ) {
				return 'script';
			}
		}
	...
	/**
	 * Whether the tag callback is one of the methods that intentionally return HTML.
	 *
	 * @param callable $function The value-producing callback registered for the tag.
	 *
	 * @return bool
	 */
	protected function is_html_tag_function( $function ) {
		return is_array( $function ) && isset( $function[1] ) && in_array( $function[1], $this->get_html_tag_methods(), true );
	}

	/**
	 * Value-producing methods that intentionally return HTML markup and must NOT be
	 * HTML-escaped when substituted in HTML body context. Each of these either
	 * sanitises its own output (wp_kses_post) or returns trusted, plugin-generated
	 * markup. In attribute or script context they are HTML-escaped regardless.
	 *
	 * @return array
	 */
	protected function get_html_tag_methods() {
		return apply_filters(
			'wpcode_smart_tags_html_methods',
			array(
				'tag_value_content',                      // Post content + Woo/MemberPress descriptions (wp_kses_post'd).
				'tag_value_wc_product_price_html',        // WooCommerce price markup.
				'tag_value_wc_product_short_description', // WooCommerce short description (wp_kses_post'd).
			)
		);
	}
	...
	public function get_shortcode_attribute_value( $parameters = array() ) {
		if ( isset( $parameters['snippet'] ) && isset( $parameters['smart_tag_key'] ) && $parameters['snippet'] instanceof WPCode_Snippet ) {
			...
			if ( isset( $snippet->attributes[ $key ] ) ) {
				// Escaping happens centrally in replace_tag_occurrences().
