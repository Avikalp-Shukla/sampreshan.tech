		...
		if ( in_array( $cap, $blocks_capabilities, true ) ) {
			...
			if ( ! empty( $custom_capability_needed ) ) {
				...
				if ( $this->user_has_role_in_roles( $capability_roles, $user_id ) ) {
					$object_write_caps = array( 'edit_wpcodeblock', 'delete_wpcodeblock' );
					if ( in_array( $cap, $object_write_caps, true ) && ! empty( $args[0] ) && ! user_can( $user_id, 'manage_options' ) ) {
						$block_post = get_post( $args[0] );
						if ( ! $block_post || (int) $block_post->post_author !== (int) $user_id ) {
