...
class WPCode_My_Library extends WPCode_Library {
	...
	/**
	 * Whether to stamp the connected username as the library author on import.
	 *
	 * @var bool
	 */
	protected $stamp_library_author = true;
	...
	/**
	 * Stamp the connected username on imported snippets so the badge can attribute them.
	 *
	 * @param int $library_id The cloud id of the snippet to import.
	 *
	 * @return false|WPCode_Snippet
	 */
	public function create_new_snippet( $library_id ) {
		$snippet = parent::create_new_snippet( $library_id );

		if ( $this->stamp_library_author && $snippet && $snippet->get_id() > 0 ) {
			$auth_username = wpcode()->library_auth->get_auth_username();
			if ( ! empty( $auth_username ) && empty( get_post_meta( $snippet->get_id(), '_wpcode_library_author', true ) ) ) {
				update_post_meta( $snippet->get_id(), '_wpcode_library_author', sanitize_key( $auth_username ) );
			}
		}
