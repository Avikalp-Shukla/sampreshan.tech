...
function wpcode_load_revisions_compare() {
	...
	} elseif ( ! current_user_can( 'edit_post', $snippet_id ) ) {
		wp_send_json_error();
	}
...
function wpcode_verify_license() {
	...
	// Check for permissions. The flag comes from the request as is_network_admin() is always false in admin-ajax.php.
	$multisite = is_multisite() && isset( $_POST['multisite'] ) && boolval( $_POST['multisite'] );
	if ( ! current_user_can( $multisite ? 'manage_network_options' : 'manage_options' ) ) {
